...
function buscar_proveedores($buscar) {
    ...
    $sSql = 'SELECT '
            ...
            . ' p.nombre '
            . 'FROM '
            . ' proveedores p, registros r '
            . 'WHERE '
            . ' p.id = r.id AND elim=0 AND '                   
            . 'nombre like "%' . $buscar['buscar'] . '%"' 
            . 'ORDER BY r.autorizado, p.nombre';
...
function buscar_proveedores_estatus($buscar) {
...
function buscar_proveedores_sitio($buscar) {
...
function get_last_proveedor($cantidad = 1000) {
    $sSql = 'SELECT '
            . ' p.id,'
            . ' r.pagado, ' 
            . ' r.autorizado, '
   ...
   if(!$data){
   		set_flash('<strong>¡No hay proveedores aun!</strong> <br />Talvez quieras agregar algunos cuantos...', 'info');
   }
...
function salvar_proveedor($aData) {
    ...
    if (isset($aData['id'])) {
                ...
                $result = db_modificar('proveedores',$aData['id'] , array(
                    'id' => (int)$id,
        ...
        $exit = "Proveedor exitosamente <strong>actualizado</strong>";
    } else {
        ...
        $exit = "Proveedor exitosamente <strong>creado</strong>";
    }
...
function cargar_proveedores_exportar( ) {
    ...
    $sSql = 'SELECT '
            ...
            . ' u.municipio '
            . 'FROM '
            . ' proveedores p, registros r, catalogos_proveedores c, estados e, municipios u '
            . 'WHERE '
            . ' p.id = r.id AND p.categoria_id = c.id AND p.estado_id = e.id AND p.municipio_id = u.id AND r.elim=0 '            
            . 'ORDER BY p.nombre';
...
function proveedores_aseguradoras($id) {
...
function proveedores_tarjetas($id) {
